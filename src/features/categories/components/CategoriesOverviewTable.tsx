...
import { EditCategoryModal } from "@/features/categories/components/EditCategoryModal.tsx";
...
import { useGetCategories } from "@/features/categories/hooks/useGetCategories.ts";
...
export function CategoriesOverviewTable() {
  ...
  return (
    <>
      ...
      >
        {(category) => (
          <TableRow key={category.categoryId}>
            ...
            <TableCell>
              <span className={`${!category.description ? "text-content4-foreground" : ""}`}>
                {category.description || "No description"}
              </span>
            </TableCell>
            <TableCell>
              <span className={`${!category.superCategoryName ? "text-content4-foreground" : ""}`}>
      ...
      {editingCategory && (
        <EditCategoryModal editingCategory={editingCategory} isOpen={isOpen} onClose={onClose} />
      )}
    </>
  );
}
