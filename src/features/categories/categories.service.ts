...
import { CategoryResponse } from "@/features/categories/interfaces/responses/category-response.interface.ts";
import { CreateCategoryRequest } from "@/features/categories/interfaces/requests/create-category.interface.ts";
import { PaginationRequest } from "@shared/interfaces/pagination/pagination-request.interface.ts";
import { UpdateCategoryRequest } from "@/features/categories/interfaces/requests/update-category.interface.ts";
...
export async function getDeletedCategories(params: PaginationRequest) {
  const response = await httpClient.get<CategoryResponse>(`${CATEGORIES_URL}/deleted`, {
    params,
  });
  return response.data;
}
...
export async function updateCategory(params: UpdateCategoryRequest) {
  const response = await httpClient.put<Category>(`${CATEGORIES_URL}/${params.categoryId}`, {
    ...params,
  });
  return response.data;
}
