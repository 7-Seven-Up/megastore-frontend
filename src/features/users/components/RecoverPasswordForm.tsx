...
} from "@/features/users/schemas/recover-password.schema.ts";
import { Button } from "@nextui-org/react";
import { useRecoverPassword } from "@/features/users/hooks/useRecoverPassword.ts";
...
export default function RecoverPasswordForm({ token }: RecoverPasswordProps) {
  ...
  const { control, handleSubmit } = useForm<ChangePasswordRecoverPasswordSchemaType>({
    resolver: zodResolver(RecoverPasswordSchema),
  });

  const onSubmit = ({ password, confirmPassword }: ChangePasswordRecoverPasswordSchemaType) => {
    mutate({
