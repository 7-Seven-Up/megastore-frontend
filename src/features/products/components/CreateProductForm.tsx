...
} from "@/features/products/schemas/create-product.schema.ts";
...
import { useCreateProduct } from "@/features/products/hooks/useCreateProduct.ts";
import { SizesSelect } from "@/features/sizes/components/SizesSelect.tsx";
...
export function CreateProductForm({ onClose }: CreateProductFormProps) {
  ...
  return (
    <form className={"grid grid-cols-12 gap-2"} onSubmit={handleSubmit(onSubmit)}>
      ...
      <FormFooter className={"col-span-12"} isLoading={isCreating} onClose={onClose} />
    </form>
  );
}
